...
def generate_loss_picture(x_array, loss_train_array, loss_test_array, data_number):
    """
    生成损失函数随次数变化图片
    :param data_number: 训练集的数据量
    :param x_array: 横坐标向量
    ...
    plt.ylabel('loss')
    plt.legend(loc=1)
    title = 'loss of different poly degree, data number = ' + str(data_number)
    plt.title(title)
